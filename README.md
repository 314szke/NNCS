...
The goal of this project is to create neural network controllers that can follow the control actions of nominal controllers as closely as possible. Comparing individual actions is straightforward as we can measure the tracking performance by the distance of the two actions in the control space. Since controllers manage the system under control over time, we need a proper performance metric to express the closeness of the trajectories over time. Currently we use Mean Squared Error (MSE) performance function, which provides equal importance for each trace point. The average changes smoothly during the neural network training and results in a rather fast training. However, averaging hides crucial information about the actual performance of the neural network (worst case scenarios) and easily results in overfitting the network.

Since neural networks learn by trying to find the closest prediction for each input data point and the overall performance of the network is the average of those predictions, repeating data points have higher importance. In a closed-loop system repetitions usually occur during steady state segments of the system, whereas our interest is in handling changes. We implemented two ways to give weight for state segments with change. We preprocess the input data and trim consecutive repeating data points based on a distance criteria. This allows the neural network to treat all input data points more equally during training, as the average error will be less biased. Additionally, we added weights for each data point. This potentially decreases the performance of the neural network for data points with lower weights, but gives more guarantee, meaning that if the performance metric of the neural network is low after training, the network probably performs well in the critical segments.

Finding the right trimming criteria and the proper weight function can be troublesome and requires manual tuning for each use case. A possible future work for this project is to use diagnostic techniques to guide the neural network training with proper weights based on the diagnostic information. With a more sophisticated trimming algorithm, the unnecessary points could be automatically determined as well.

In case the neural network controller does not satisfy the required system properties after the initial training, we retrain the network with more training data including counter-examples. Since the performance metric of the network is based on an average, we cannot give any guarantee that a smaller performance value means a better neural network controller from property satisfaction point of view. This means, that each neural network needs a falsification step to properly evaluate its performance. Additionally, the possible number of epochs for retraining is limited, since after a certain training iteration the neural network overfits.

To address these issues, the performance function used during the neural network training needs to be changed. One possibility is to use the maximum of the point-wise errors, which would provide both guarantee and useful information about the performance, but probably would result in slower training. Currently, Matlab does not support max as a performance function. It can be implemented by copying the code of the MSE performance function from the Matlab library and rewrite it locally in the workspace to calculate the maximum. An other approach would be to use Python for the neural network training and import the final weights and biases of the neural network and set it in Matlab for the shallow neural network object to provide the interface for Simulink.

Based on the initial results, it is possible to train neural networks to control rather simple systems. With proper training data preprocessing and training parameters, the network can be trained in a few iterations to achieve the satisfaction of the required properties. The project is still in an early stage and additionally to the above mentioned possibilities for future work, experimenting with complex models would bring more insight on how to improve the neural network training.
