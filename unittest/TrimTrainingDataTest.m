...
function testInvalidInputType(testCase)
    ...
    epsilon = 1;
    repetition = 1;
    testCase.verifyError(@()TrimTrainingData(in, out, trace_end_indices, epsilon, repetition), 'TrimTrainingData:TypeError');
end
...
function testEmptyInput(testCase)
    ...
    epsilon = 0.1;
    repetition = 2;
    testCase.verifyError(@()TrimTrainingData(in, out, trace_end_indices, epsilon, repetition), 'TrimTrainingData:EmptyInput');
end
...
function testDifferentDimensions(testCase)
    ...
    epsilon = 0.1;
    repetition = 3;
    testCase.verifyError(@()TrimTrainingData(in, out, trace_end_indices, epsilon, repetition), 'TrimTrainingData:InputMismatch');
end
...
function testInvalidEpsilon(testCase)
    ...
    epsilon = -0.1;
    repetition = 6;
    testCase.verifyError(@()TrimTrainingData(in, out, trace_end_indices, epsilon, repetition), 'TrimTrainingData:InvalidInput');
end
...
function testNoTrimming(testCase)
    ...
    [new_in, new_out, new_indices] = TrimTrainingData(in, out, trace_end_indices, epsilon, repetition);
